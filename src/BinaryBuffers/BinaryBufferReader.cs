...
public sealed class BinaryBufferReader : IBufferReader
{
    ...
    /// <summary>
    /// Reads an unmanaged value of type <typeparamref name="T"/> from the underlying byte array and advances the current position by the size of <typeparamref name="T"/>.
    /// </summary>
    ...
    /// <summary>
    /// Reads enough unmanaged values of type <typeparamref name="T"/> to fill <paramref name="destination"/> from the underlying byte array and advances the current position by the total number of bytes read.
